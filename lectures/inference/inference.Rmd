...
1. We know that the average of the sampling model is -1/19 and the standard deviation is $\sigma = 2 \sqrt{10/19 \times 9/19}$. Run a Monte Carlo simulation with $N=400$ to learn about the distribution of the errors. Remember the error is the average winning minus $\mu = -1/19$.Confirm that the expected value of the error is 0 and that the standard error is $\sigma / \sqrt{N}$. http://goo.gl/forms/YHQgejqKfl

```{r}

```

2. Confirm that the distribution is approximately normal with mean 0 and standard deviation $\sigma / N$ http://goo.gl/forms/5iET73x0dR
...
3. How large does the error have to be for the casino to lose money? http://goo.gl/forms/q7Ov6hl5le
...
4. Use the CLT to compute the probability of a positive earnings. Confirm with the simulation results. http://goo.gl/forms/7TWqQdrWuf

```{r}
pnorm
```

5. With $N=400$ the probability of negative earnings is too high. What does $N$ need to be for the probability of positive earnings is 0? Confirm with a simulation. http://goo.gl/forms/2uRkCr5nHI
...
1. What is the average and standard deviation of our sampling model? What is the expected value and standard deviation. http://goo.gl/forms/0NfIk7PkpC
...
2. Consider the random variable defined by taking a sample of size $N$ and then computing the average. What are the expected value and standard error of this random variable? http://goo.gl/forms/zradVEY58t
```{r}
N <- 25
mu
sigma/sqrt(N)
...
3. What is the probability that our estimate is within one inch of the population mean? http://goo.gl/forms/zIiqhGuVCl

```{r}
1-(1-pnorm(1/(sigma/sqrt(N))))*2
...
$\mbox{Pr}(\mid Z \mid \leq z) = 0.95$? http://goo.gl/forms/XmMeUchfz5
...
Did we actually make a correct prediction? It's election night and we we will find out. http://goo.gl/forms/xiMwzrIC1J
...
#### Assessment
Construct a 99% confidence interval. Is it more or less specific than the 95%? http://goo.gl/forms/8kCxwTptIX
```{r}
...
```{r}
ph_hat <- mean(sample(c(1,0),100, replace=T, prob=c(prob_heads,1-prob_heads)))
N <- 100
conf_int <- ph_hat + c(-1,1)*qnorm(0.975)*sqrt(ph_hat*(1-ph_hat)/N)
conf_int
```


3. Does the interval cover the true value of `prob_head`? http://goo.gl/forms/jXeOvBcAJg


4. No use a Monte Carlo simulation to re run this experiment (keeping `prob_hear` fixed) 10000 times. Do we confirm that we cover 95% of the time? http://goo.gl/forms/GqHUcZkSDM

```{r}
cover <- replicate(10000, {
        ph_hat <- mean(sample(c(1,0),100, replace=T, prob=c(prob_heads,1-prob_heads)))
        conf_int <- ph_hat + c(-1,1)*qnorm(0.975)*sqrt(ph_hat*(1-ph_hat)/100)
...
What percent of confidence intervals with margins of error of 0.03 would correctly predict the election? Check with a Monte Carlo simulation. http://goo.gl/forms/jKADt1Q9d5

### Assessment

If we know the percent of democrats can be as close to 50% as, say, 48%, how large does the sample size have to be so that the margin of error is about 2%. http://goo.gl/forms/xi4Gp5FoIZ
...
From the `polls` object determine the sample size from the margin of error. Then plot it against the reported sample size. 
http://goo.gl/forms/bDQNcfHQbQ
