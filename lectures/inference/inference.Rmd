...
Assessment: Repeat the exercise of taking a poll of 625 likely voters, 10,000 times. Without peaking at $p$ study the distribution of $\hat{p}-p$. Have we seen this distribution before? How often was our error larger than 0.02? [http://goo.gl/forms/JqyeuvvJiR](http://goo.gl/forms/JqyeuvvJiR)

```{r}
B <- 10^4
N <- 625
error <- replicate (B, {
        phat <- mean(sample(x, N, replace=T))
        phat-p
        })
mean(abs(error)>0.02)
