...
Rs <- replicate(B, sample_n( father.son, N, replace=TRUE) %>% 
            summarize(r = cor(fheight, sheight)) %>% .$r
          )

dat <- data.frame(R = Rs - r)
ggplot(dat, aes(Rs)) + geom_histogram()
...
Re-do the assessment when the sample size is 250 instead of 25?
...
1. We use $\gamma$ instead of $\beta$ and $\delta$ instead of $\varepislon$ because this is a different model. We can't solve the first equation to get the second because $\varepsilon$ is a random variable. 
