...
```{r}
tmp <- slice(original_dat,1:100)   # run in a smaller set to test out
...
tmp <- tmp %>% gather(feature, value, `0`:`783`) # transform to long form
...
tmp <- tmp %>% mutate(row = floor(row/4), col = floor(col/4))     # get new row and col
...
plot(table(round(column_means)))  # about 10 always have 0
```
...
plot(res)    # CV tells 3 have the highest accuracy
